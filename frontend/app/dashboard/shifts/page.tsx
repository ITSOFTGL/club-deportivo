...
export default function ShiftsPage() {
  ...
  const filteredShifts = shifts.filter((shift) => {
    ...
    return shift.name?.toLowerCase().includes(search);
  });
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Gestiona los horarios y turnos del club</p>
        </div>
        <button
          onClick={() => {
            setEditingShift(null);
      ...
      <DeleteConfirmModal
        ...
        message={`¿Estás seguro de eliminar "${deletingShift?.name}"?`}
        loading={formLoading}
      />
    </motion.div>
  );
